	       ...
	       float *MeltEnergy, float *Shear, float *IceA, float *IceV, float Slope)
{
  ...
  float Ao;			/* Empircal consistency of ice term, for glacier movement */
  float Tau;
  ...
  *Swq = Ice + *PackWater + *SurfWater;
  ...
  /* Calculate basal shear stress, ice consistancy, ice depth and velocity;
        Equation from "The Physics of Glaciers" W.S.B.Paterson, page240, eq1 */
  if ( *Swq > 0 ) {
    *Shear = ICE_DENSITY * G * (*Swq) * (WATER_DENSITY/ICE_DENSITY) * sin(Slope);
    Ao = (3.2 + 5.8 * ((*PackWater)/(*Swq) * 100)) * 10e-15;  /*  Duvel, 1977, from Paterson, page87 */
    *IceA = Ao * exp(-GLACIER_Q/((UNIGASK*101.324)*(*TPack+273.15)));  // UNIGASK converted to KJ/mol/K
  
    if( *Shear > EactAir ) {
       Tau = *Shear - EactAir;
       *IceV = ((2 * (*IceA))/(GLACIER_N + 2)) * pow((Tau),GLACIER_N) * ((*Swq) * (WATER_DENSITY/ICE_DENSITY)); //Patterson, p243, eq6
    } else {
      *IceV = 0.0;
    }
  } else {
