 ...
 * COMMENTS:
 * $Id: CanopyResistance.c,v 1.1.1.1 2002/09/24 04:58:49 nijssen Exp $     
 */
...
float CanopyResistance(float LAI, float RsMin, float RsMax, float Rpc,
		       ...
		       float TSoil, float SoilMoisture_m, float Vpd, float Rp)
{
...
/*   TFactor = 1.0/(.0705 * TSoil - 0.0013 * pow(TSoil, (double) 2.0)); */
  ...
  if (SoilMoisture_m <= WP) {
    Resistance = DHSVM_HUGE;
    return Resistance;
  }
  else if (SoilMoisture_m < MoistThres)
    MoistFactor = (MoistThres - WP) / (SoilMoisture_m - WP);
  else
    MoistFactor = 1.0;
