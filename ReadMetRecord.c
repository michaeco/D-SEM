 ...
 * COMMENTS:
 * $Id: ReadMetRecord.c,v 1.1.1.1 2002/09/24 04:58:50 nijssen Exp $     
 */
		   ...
		   MET * MetRecord, BASINWIDE * Basinwide)
{
  ...
  if (ScanFloats(InFile->FilePtr, Array, NMetVars) != NMetVars) {
    printf("Expecting %d variables in met record but found only %d\n",NMetVars, ScanFloats(InFile->FilePtr, Array, NMetVars));
    ReportError(InFile->FileName, 5);
    }
  ...
  if (Options->PrecipLapse == VARIABLE) {
    ...
    MetRecord->PrecipLapse = Basinwide->PrcpLapse[Current->Month - 1];
  } else
    MetRecord->PrecipLapse = PRECIPLAPSE;

  /* Added ability to have "Constant TLR, but vary by month with TLR option set to  MONTLY */  
  if (Options->TempLapse == VARIABLE) {
    MetRecord->TempLapse = Array[5 + i];
    i++;
  }
  else if (Options->TempLapse == MONTHLY) {
    MetRecord->TempLapse = Basinwide->TempLapse[Current->Month - 1];
    //printf("TLR for month %d read as %f\n",Current->Month,MetRecord->TempLapse);
  }
  else
