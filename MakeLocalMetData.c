 ...
 * COMMENTS:
 * $Id: MakeLocalMetData.c,v 1.2 2002/09/25 05:29:10 nijssen Exp $     
 */
			...
			float SineSolarAltitude)
{
  ...
  if (Options->MM5 == TRUE && Options->QPF == TRUE) {
    WeightSum = 0.0;
    for (i = 0; i < NStats; i++)WeightSum += (float) MetWeights[i];
  }

  if (Options->MM5 == TRUE) {
    LocalMet.Tair = MM5Input[MM5_temperature - 1][y][x] +(LocalElev - MM5Input[MM5_terrain - 1][y][x]) * MM5Input[MM5_lapse -1][y][x];
    ...
    if (Options->Shading == TRUE) {
		if (SunMax > 0.0)SeparateRadiation(LocalMet.Sin, LocalMet.Sin / SunMax,&(LocalMet.SinBeam), &(LocalMet.SinDiffuse));
		else {
    ...
    for (i = 0; i < NStats; i++) {
      ...
      LocalMet.Tair += CurrentWeight *LapseT(Stat[i].Data.Tair, Stat[i].Elev, LocalElev,Stat[i].Data.TempLapse);
  ...
  if (LocalMet.Press<0)assert(FALSE);
  ...
  if (LocalSnow->HasSnow) {
   ...
   (*MetMap)[y][x].accum_precip = (*MetMap)[y][x].accum_precip + PrecipMap->Precip;
