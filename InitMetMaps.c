 ...
 *               InitRadMap()
 * COMMENTS:
 * $Id: InitMetMaps.c,v 1.5 2002/11/19 17:07:59 nijssen Exp $     
 */
		 ...
		 float ****WindModel)
{
  ...
  InitEvapMap(Map, EvapMap, SoilMap, Soil, VegChemMap, Veg, TopoMap);
  InitPrecipMap(Map, PrecipMap, VegChemMap, Veg, TopoMap);
		 ...
		 TOPOPIX **TopoMap)
{
  ...
  for (y = 0; y < Map->NY; y++) {
	...
	assert(VegChemMap[y][x].Veg > 0 && SoilMap[y][x].Soil > 0);
	...
	if (!((*EvapMap)[y][x].ESoil_m =
	      (float **) calloc(NVeg, sizeof(float *))))
	  ReportError((char *) Routine, 1);

	for (i = 0; i < NVeg; i++) {
	  if (!((*EvapMap)[y][x].ESoil_m[i] =
...
{
  const char *Routine = "InitMetMap";
  int y;			/* counter */

  if (DEBUG)
    printf("Initializing Met Pixel Map map\n");
