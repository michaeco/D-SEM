 ...
 * COMMENTS:
 * $Id: CalcTransmissivity.c,v 1.3 2002/10/02 00:36:03 nijssen Exp $     
 */
			 ...
			 float KsExponent)
{
  ...
  if (fequal(KsExponent, 0.0))Transmissivity = LateralKs * (SoilDepth - WaterTable);
  else Transmissivity = (LateralKs / KsExponent) * (exp(-KsExponent * WaterTable) - exp(-KsExponent * SoilDepth));
