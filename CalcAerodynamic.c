 ...
 * COMMENTS:
 * $Id: CalcAerodynamic.c,v 1.2 2002/09/25 05:29:09 nijssen Exp $     
 */
		     ...
		     float *U2mSnow, float *Ra, float *RaSnow)
{
  ...
  /* Overstory present, one or two vegetation layers possible */
    ...
    /* MWW error catch added */

    if(isnan(Ra[0])) fprintf(stderr, "Warning Ra[0] = nan! Check reference height or wind attenuation veg factors\n");
	if(isinf(Ra[0]))
	{
		assert(FALSE);
	}
    ...
    /* case 2: the wind profile to a height of 2m above the lower boundary 
      ...
      Ra[1] =
	log((Zt - d_Lower) / Z0_Lower) * log((Zt - d_Lower) / Z0_Lower) / (K2 * Ut) +
			Height[0] * log((Zref - d_Upper) / Z0_Upper) / (n * K2 *
			(Zw - d_Upper)) * (exp(n *(1 - Zt/Height[0])) -
			exp(n * (1 - (Z0_Lower + d_Lower + 2.) / Height[0])));
    }
	...
	(K2 * Ut) + Height[0] * log((Zref - d_Upper) / Z0_Upper) / (n * K2 * (Zw - d_Upper)) * (exp(n *
	 (1 - Zt / Height[0])) - exp(n * (1 - (Z0_SNOW + 2.) / Height[0])));
    }
    ...
    /* case 3: the top of the overstory is less than 2 m above the lower boundary.
      ...
      *U2mSnow = Uh;
      *RaSnow = log(Zt / Z0_SNOW) * log(Zt / Z0_SNOW) /
	(K2 * Ut) + Height[0] * log((Zref - d_Upper) / Z0_Upper) / (n * K2 * (Zw - d_Upper)) 
