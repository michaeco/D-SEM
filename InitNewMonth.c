 ...
 *               InitNewStep()
 * COMMENTS:
 * $Id: InitNewMonth.c,v 1.6 2002/11/19 17:07:59 nijssen Exp $     
 */
...
//#ifndef NO_DIAG
 // if ( Options->TempLapse == MONTHLY )     
  //  printf("reading in new Temperature Lapse Rate field for month %d: %f \n", 
    //        Time->Current.Month, Basinwide->TempLapse[Time->Current.Month - 1]);
// #endif    
...
//
  ...
  if (Options->Prism == TRUE) {
	sprintf(FileName, "%s.%02d.bin", Options->PrismDataPath, Time->Current.Month);
  ...
  for (i = 0; i < NVegs; i++) {
    ...
    if (VType[i].OverStory) {
      ...
      if (l == 0)VType[i].Taud = 1.0;
      else VType[i].Taud =  exp(-b * l) * ((1 - a * l) * exp(-a * l) + (a * l) * (a * l) * evalexpint(1, a * l));
    }
    else VType[i].Taud = 0.0;
   
  }//for i = 0 to nvegs

  /*  If using nonpoint sources, read current months population raster */
 // printf("Number of NPS categories %d\n", NpsCats);
  if ( NpsCats > 0 ) {
    printf("Population for month %d, %d", Time->Current.Month,Time->Current.Year);
    sprintf(FileName, "%s.%02d.bin", Options->PopulationDataPath, Time->Current.Month);
	GetVarName(205, 0, VarName);
    GetVarNumberType(205, &NumberType);
    if (!(Array = (float *) calloc(Map->NY * Map->NX, sizeof(float))))
		ReportError((char *) Routine, 1);
    ...
    for (y = 0; y < Map->NY; y++) {
		for (x = 0; x < Map->NX; x++) {
			PopulationMap[y][x] = Array[y * Map->NX + x];/// r45 was *20.5 //r40 was *10 //r39 was *1  //r38 was *10 //jasons //HARDCODED
			*TotalPopulation += PopulationMap[y][x];
		}
    }
    free(Array);
    printf(" is %i\n", *TotalPopulation);
...
/*end if MM5*/
  ...
  if (Options->FlowGradient == WATERTABLE) {
	...
	if (INBASIN(TopoMap[y][x].Mask)) {
	  SoilMap[y][x].WaterLevel =
	    TopoMap[y][x].Dem - SoilMap[y][x].TableDepth_m;
	}
      }
    }
  ...
  if ((Options->MM5 == TRUE && Options->QPF == TRUE) || Options->MM5 == FALSE)
    GetMetData(Options, Time, NSoilLayers, NStats, SolarGeo->SunMax, Stat,
	       Radar, RadarMap, RadarFileName, Basinwide);
