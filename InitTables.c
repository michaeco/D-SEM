 ...
 *               InitSnowTable()
 * COMMENTS:
 * $Id: InitTables.c,v 1.7 2002/11/19 17:07:59 nijssen Exp $     
 */
...
int InitSoilTable(SOILTABLE ** SType, LISTPTR Input, LAYER * Soil)
{
  ...
  for (i = 0; i < NSoils; i++) {
    ...
    if (!CopyFloat(&((*SType)[i].MaxInfiltrationRate), VarStr[infiltration], 1))
     ...
     if (!CopyFloat(&((*SType)[i].TemperatureExponent), VarStr[temperature_exponent], 1))
      ReportError(KeyName[temperature_exponent], 51);

      if (!CopyFloat(&((*SType)[i].ThermalInertia), VarStr[thermal_inertia], 1))
      ReportError(KeyName[thermal_inertia], 51);
...
/*      printf("\t\tFinding %s %d\n", KeyStr[j], i + 1);  */
    ...
    if (!((*VType)[i].RootDepth_m = (float *) calloc((*VType)[i].NSoilLayers, sizeof(float))))
	       ReportError((char *) Routine, 1);
    ...
    if (!CopyFloat((*VType)[i].RootDepth_m, VarStr[root_zone_depth],
		   (*VType)[i].NSoilLayers))
